...
- teacher forcing is used so no models have to deal with their own noise
- finite differences due to difficulties w/ backprop through entanglement
  - truncated (only single, shared, calculation for first point in all gradient calculations for that epoch) to increase training speed
